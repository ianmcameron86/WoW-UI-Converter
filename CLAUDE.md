...
- **Install folders:** Ian's install is at `D:\BattleNet\World of Warcraft` and has exactly four: `_retail_`, `_anniversary_`, `_classic_beta_` and `_classic_era_`. The WoW Forever beta is `_classic_beta_`, but **that folder name is misleading**: the in-game probe reported `WOW_PROJECT_ID = 1`, i.e. **MAINLINE**, not Classic (it's version 1.60.1, tocversion 16001). Don't infer a client's code base from its folder name. **His Burning Crusade Classic is `_anniversary_`** — there is no `_classic_` folder. Checked directly on his PC on Sept 24, 2026.
...
- **The server copy really does wipe a copied keybind file. Confirmed.** Ian's hand-copied Forever keybinds worked at first, then disappeared in-game. On disk, `_classic_beta_\...\bindings-cache.wtf` is **0 bytes** and `bindings-cache.old` holds the copy (54 binds, identical to his current Retail file apart from line order). The timestamps show the copy at 20:51 and WoW rotating it to `.old` and writing an empty file at 20:54, on logout. `synchronizeBindings` was **not set** in any of the four `Config.wtf` files, because he copied by hand rather than using the tool.
- **So `SET synchronizeBindings "0"` is not optional**, it's what makes a keybind copy survive. The tool's checkbox is on by default, which would have prevented this. Don't let anyone talk it down to a "fallback".
